...
const ManagementUser = () => {
  ...
  const {
    ...
    queryKey: ["user"],
    queryFn: async () => {
      const res = await axiosCommon.get("/allUser");
    //   console.log(res.data);
      return res.data;
    },
  });
  const handleToggleStatus =  async(userId, role, status, button) => {
       const respond= await axiosCommon.patch(`/allUser/${userId}/status`,{role, status, button})
    //    console.log(respond.data);
    //     console.log(userId,role,button)
        if (respond.data.modifiedCount) {
            refetch();
        }
};
  ...
  return (
    <div>
      ...
      <div className="relative pb-5 my-7">
                ...
                {isPending ? (
                  ...
                  users?.map((user) => (
                    <tr key={user._id} className="*:text-center *:min-w-32">
                      <td className="py-2 px-4 border-b">  {user.name.slice(0, 1).toUpperCase() +
                              user.name.slice(1)}</td>
